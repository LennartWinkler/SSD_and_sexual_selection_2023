...
Additional analyses excluding studies that did not correct for phylogenetic non-independence (see Supplement 2).
...
```{r Global non-phylogenetic Model V1, warning=FALSE, message=FALSE}
# Non-phylogenetic Model
Model_cREML_Null         = rma.mv(absSSDcor ~ 1, V=Var_r, data = MetaData, random = c(~ 1 | Study_ID,~ 1 | Index), method = "REML")
summary(Model_cREML_Null)
```

# Global models testing if an increase in sexual selection correlates with an increasingly males-biased SSD
...
First, we ran a global model including the phylogeny:
```{r Global phylogenetic Model V2, warning=FALSE, message=FALSE}
Model_REML_Null         = rma.mv(r ~ 1, V=Var_r, data = MetaData, random = c(~ 1 | Study_ID,~ 1 | Index, ~ 1 | Class), R = list(Class = forcedC_Moderators), method = "REML")
summary(Model_REML_Null)
```

Second, we ran a global model without the phylogeny:
```{r Global non-phylogenetic Model V2, warning=FALSE, message=FALSE}
Model_cREML_Null         = rma.mv(r ~ 1, V=Var_r, data = MetaData, random = c(~ 1 | Study_ID,~ 1 | Index), method = "REML")
summary(Model_cREML_Null)
```
...
ggplot(MetaData_SexBias, aes(x=SSD_SexBias_in_perc_F, y=r)) +
...
## Plot Type of SSD measure (Figure S2B)

Here we plot the type of SSD measure moderator:
```{r Figure S2B, warning=FALSE, message=FALSE, fig.align="left",fig.width = 8,fig.height=7, results='hide', fig.fullwidth=TRUE,fig.cap='Figure S2B: Raincloud plot of correlation coefficients for different types of SSD measures (i.e. body mass or size) including sample sizes and estimates with 95%CI from phylogenetic model.'}
...
Model_REML_PublBias         = rma.mv(z ~ SE_z, V=rep(mean(MetaData$SE_z),length(MetaData$SE_z)), data = MetaData, random = c(~ 1 | Study_ID,~ 1 | Index, ~ 1 | Class), R = list(Class = forcedC_Moderators), method = "REML",control=list(rel.tol=1e-8))
summary(Model_REML_PublBias)
```

## Figure S2D
```{r Figure S2D, warning=FALSE, message=FALSE, fig.align="left",fig.width = 7,fig.height=5, results='hide', fig.fullwidth=TRUE,fig.cap='Figure S2D: Scatter plot of the standard error in z scores of each study against the z score of each primary study (transformed correlation coefficients). Dashed line marks a correlation coefficient of zero, black line represents predictions from REML model (controlling for phylogeny) and grey area represents 95% CI on model predictions.'}
...
## Plot taxa number (Figure S2E)

Here we plot the taxa number:
```{r Figure S2E, warning=FALSE, message=FALSE, fig.align="left",fig.width = 7,fig.height=5, results='hide', fig.fullwidth=TRUE,fig.cap='Figure S2E: Scatter plot of correlation coefficients against the natural logarithm of the number of taxa included in each study. Dashed line marks a correlation coefficient of zero, black line represents predictions from REML model (controlling for phylogeny) and grey area represents 95% CI on model predictions.'}
# Extract model predictions
...
model_p3 <- predict(Model_REML_by_TaxaNumber)
...
ggplot(MetaData, aes(x=log(as.numeric(TaxaNumber)), y=r)) +
  ...
  geom_hline(yintercept=0, linetype="dashed", color = "black", linewidth=1)+
  geom_line( aes(y = model_p3$pred), size = 1)+ 
  geom_ribbon( aes(ymin = model_p3$ci.lb, ymax = model_p3$ci.ub, color = 'black',linetype=NA), alpha = .15,show.legend = F, outline.type = "both") +
  theme
```
...
## Plot publication year (Figure S2F)

Here we plot the publication yea:
```{r Figure S2F, warning=FALSE, message=FALSE, fig.align="left",fig.width = 7,fig.height=5, results='hide', fig.fullwidth=TRUE,fig.cap='Figure S2F: Scatter plot of correlation coefficients against the publication year of each study. Dashed line marks a correlation coefficient of zero, black line represents predictions from REML model (controlling for phylogeny) and grey area represents 95% CI on model predictions.'}
# Extract model predictions
Model_REML_by_Year = rma.mv(r ~ Year, V=Var_r, data = MetaData, random = c(~ 1 | Study_ID,~ 1 | Index, ~ 1 | Class), R = list(Class = forcedC_Moderators), method = "REML")
...
Model_cREML_PublBias         = rma.mv(z ~ SE_z, V=rep(mean(MetaData$SE_z),length(MetaData$SE_z)), data = MetaData, random = c(~ 1 | Study_ID,~ 1 | Index), method = "REML")
